...
import { TRACKS as NEW_TRACKS, RAW as NEW_RAW } from "./src/new_data.js";
import { OLD_TRACKS, OLD_RAW } from "./src/old_data.js";
...
const NEW_PLAN = buildPlan(NEW_RAW);
const OLD_PLAN = buildPlan(OLD_RAW);
...
export default function App() {
    ...
    const [planType, setPlanType] = useState("new");
    ...
    useEffect(() => {
        try {
            ...
            const savedPlan = localStorage.getItem("vtask_plan_type");
            if (savedPlan) setPlanType(savedPlan);
        } catch (e) {}
    }, []);
    ...
    const changePlan = (type) => {
        setPlanType(type);
        localStorage.setItem("vtask_plan_type", type);
        setWeek(1);
        setToast(`Switched to ${type === 'new' ? '90-Day Tech' : 'Standard 10-Week'} Plan`);
    };

    const RAW = planType === "new" ? NEW_RAW : OLD_RAW;
    const TRACKS = planType === "new" ? NEW_TRACKS : OLD_TRACKS;
    const PLAN = planType === "new" ? NEW_PLAN : OLD_PLAN;
    const totalWeeks = planType === "new" ? 13 : 10;
                ...
                {tab === "profile" && (
                    <div className="page">
                        ...
                        {[
                            ...
                            { label: planType === "new" ? "Switch to 10-Week Plan" : "Switch to 90-Day Plan", icon: "🔁", action: () => changePlan(planType === "new" ? "old" : "new") },
                                    ...
                                    <div className="task-card" style={{ cursor: "default" }}>
                                        ...
                                        {detailTask.res && detailTask.res.length > 0 && (
                                            <div style={{ marginTop: 16 }}>
                                                <h4 style={{ fontSize: 13, color: "var(--text)", marginBottom: 8 }}>Resources</h4>
                                                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                                                    {detailTask.res.map((r, i) => (
                                                        <a key={i} href={r} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "var(--accent)", textDecoration: "none", background: "var(--card2)", padding: "4px 8px", borderRadius: 6, border: "1px solid var(--border)" }}>Link {i + 1}</a>
                                                    ))}
                                                </div>
                                            </div>
                                        )}
